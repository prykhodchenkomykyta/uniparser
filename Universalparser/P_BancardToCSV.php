...
Name: Radovan Jakus
Version: 1.2
Notes: Setup new processor
******************************/

//Production Environment
$sInputDir = "/home/erutberg/Radovan/DataPrep/IN/bancard/";
...
$sProcessedDir =  "/home/erutberg/Radovan/DataPrep/processed/bancard/";
$sProductConfigFile = "/home/erutberg/Radovan/Products_Configuration_Bancard.csv";
$sConfirmationReportDir = "/var/TSSS/Files/Reports/bancard/";
...
//Test Environment
$sInputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\in\\";
//$sOutputDir = "D:\\Production Data\\in\\";
$sOutputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\";
$sBulkOutputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\USPS\\BULK\\";
$sBulkFedexOutputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\FEDEX\\BULK\\";
$sMailOutputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\USPS\\";
$sMailMergeOutputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\MAILMERGE\\";
$sFedexOutputDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\FEDEX\\";
$sProcessedDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\in\\";
$sProductConfigFile = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\Products_Configuration_Bancard.csv";
$sConfirmationReportDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\REPORTS\\";
$sShipmentReportDir = "D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\out\\REPORTS\\";
$SerialNumberLocal ="D:\\Workspace\\TagSystem\\Parser_Plugin\\Bancard\\SerialNumberCounter.csv";
$sSerialNumberurl = "https://atlas.tagsystems.net/barcode/serial/";
            ...
            if(preg_match("/^#/", $aProductDetails, $comments))
                ...
                else
                {
                    ...
                    $aBINs[$aProducts[1]][$aProducts[2]][$aProducts[3]]=array(
                        "Profile"=>$aProducts[5],
                        "Product"=>$aProducts[4],
                        "ServiceType"=>$aProducts[8],
                        "PackageType"=>$aProducts[9],
                        "WeightOz"=>$aProducts[10],
                        "ShipDate"=>$aProducts[11],
                        "FromFullName"=>$aProducts[12],
                        "FromAddress1"=>$aProducts[13],
                        "FromAddress2"=>$aProducts[14],
                        "FromCity"=>$aProducts[15],
                        "FromState"=>$aProducts[16],
                        "FromCountry"=>$aProducts[17],
                        "FromZIPCode"=>$aProducts[18], 
                        "BulkCompanyName" => $aProducts[19], 
                        "BulkFullName" => $aProducts[20], 
                        "BulkAddress1" => $aProducts[21], 
                        "BulkAddress2" => $aProducts[22], 
                        "BulkCity" => $aProducts[23], 
                        "BulkState" => $aProducts[24], 
                        "BulkCountry" => $aProducts[25], 
                        "BulkZIPCode" => $aProducts[26], 
                        // "FEDxAccount" => $aProducts[30],
                        // "FEDEXPhoneNumber" => $aProducts[31],
                        "ShippingMethods" => array ('DEFAULT'=> $aProducts[7],
                                                   ),
...
function Parser($inputDir)
{
    ...
    // {
    //     str_replace(array("["|"]"|"+"|"$"|"*"|,"\["|"\]"|"\+"|"\$"|"\*", $sID);
    //     $rgxMICPattern .= "($sID)|"; 
    //     $rgxMICPattern = substr($rgxMICPattern,0,strlen($rgxMICPattern)-1);
    // }  

    // echo("rgxMICPattern $rgxMICPattern\n");
...
function ConfirmationReportErrorCheck($input, $inputDir)
{
    ...
    foreach($input as $iRecID => $aRecord)
    {
        ...
        $iRecordNo = 0;
        //VALIDATION DATA
        $ProductID = trim($aRecord['IDENTIFIER']);
        $CARD_STOCK_ID = "NA";
        ...
        //$SHIPPING_SERVICE = strtoupper($SHIPPING_SERVICE);
            ...
            $iRecordNo++;
            //VALIDATION DATA
            $sPAN = trim(str_replace(" ","",$aRecord['PAN_EMB']));
            ...
            $sTrack1 = trim(str_replace("]","^",($aRecord['TRACK1'])));
            $sTrack2 = trim($aRecord['TRACK2']);
            $sTrack2Chip = "";
            $iCVV = trim($aRecord['ICVV']);
            $CVV2 = trim($aRecord['CVV2']);
            $sEmbName = trim($aRecord['CARDHOLDER_NAME']);
            $sCompanyName = "\"".trim($aRecord['COMPANY'])."\"";
            $FullName = trim($aRecord['CARDHOLDER_NAME']);
            $Company =  trim($aRecord['COMPANY']);
            $Address1 = "";
            $Address2 = "";
            $City = "";
            $State = ""; 
...
function MailingInput($input, $inputDir, $outputDir)
{
    ...
    $bBulk = false;
            ...
            foreach($input as $aRecord)
            {
                ...
                $State =  ""; 
                $ZIPCode = "";
                $ZIPCodeAddOn = empty("") ? "" : preg_replace("/-/","",substr(trim($aRecord['17']),5)); //ADDED ZIPAdd-On
                $Country = "";
                $EmailAddress;
