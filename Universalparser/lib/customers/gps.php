...
******************************/
function mapping_gps($aInputFile)
{
 

    echo "MAPPING GPS";
    $aRecords = array();
    $xmlfile = file_get_contents($aInputFile);
    $aSimpleXML = simplexml_load_string($xmlfile);
    $json = json_encode($aSimpleXML); 
    $aTemp = json_decode($json,TRUE);
    $iRecNo = 0;

    if(isset($aTemp['PRODUCT']['RECORD']))
    {
        if(isset($aTemp['PRODUCT']['RECORD']['CARD']))
        {
            $aRecords[$iRecNo] =  array_merge($aTemp['PRODUCT']['RECORD'],$aTemp['CARDSUM'],array('PRODUCT_REF'=>$aTemp['PRODUCT']['PRODUCT_REF']));
            $iRecNo++;
        }
        else
        {
            foreach($aTemp['PRODUCT']['RECORD'] as $sRecordNo => $aRecordData)
            {
                $aRecords[$iRecNo] = array_merge($aRecordData,$aTemp['CARDSUM'],array('PRODUCT_REF'=>$aTemp['PRODUCT']['PRODUCT_REF']));
                $iRecNo++;
            }
        }
    }
    ...
    {
        foreach($aTemp['PRODUCT'] as $sProductName => $aProduct)
        {
            if(isset($aProduct['RECORD']['CARD']))
            {
                $aRecords[$iRecNo] =  array_merge($aProduct['RECORD'],$aTemp['CARDSUM'],array('PRODUCT_REF'=>$aProduct['PRODUCT_REF']));
                $iRecNo++;
            }
            else
            {
                foreach($aProduct['RECORD'] as $sRecordNo => $aRecordData)
                {
                    $aRecords[$iRecNo] = array_merge($aRecordData,$aTemp['CARDSUM'],array('PRODUCT_REF'=>$aProduct['PRODUCT_REF']));
                    $iRecNo++;
                }            
            }
...
function getvalue($key_string, $input) {
    $path = "";
    $path = explode('.', $key_string);
    $temp =& $input;
      foreach($path as $key) {
          $temp =& $temp[$key];
      }
...
function datamap_validation_gps($input, $inputDir, $aConfig)
{
    ...
    foreach($input as $iRecID => $aRecord)
    {
        ...
        $LastNameT1 = "";
        $ExpDate_YYMM = "";
        $ChipData = "$Track1#$Track2#$Track2Chip#$PSN#$PINBlock#$CVC2#\"$EmbName\"#$CompanyName|$Track3|$EmbossLine4";
