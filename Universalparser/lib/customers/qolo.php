...
function mapping_qolo($aInputFile)
{
...
function datamap_validation_qolo($input, $inputDir, $aConfig)
{
    ...
    foreach($input as $iRecID => $aRecord)
    {
        ...
        $CardStockID = "NA";
        $ShippingMethod  = (($OrderType == "DTC")? "ShipMethod": "BulkShipMethod"); //BULK or DTC
        
        // calculate return full-name:
        $FromFullName = $FirstNameT1 . ' ' . $LastNameT1;
...
// function datamap_validation_qolo($input, $sInputFilePath, $aConfig)
// {
//     $sDateStamp = date('Y-m-d H:i:s', time());
//     $sUser = get_current_user();
//     $sProductConfigFile = $aConfig['ProductConfigurationPath'];
...
//     echo "\n$sDateStamp [$sUser]: Mapping Data: $sInputFilePath \n";
//     $aMappedDataBeforeValidation = array();
...
//         $aMappedDataBeforeValidation = array_merge($aMappedDataBeforeValidation,mapDataFromConfig($sDataMapConfiguration, $aRecord, $sProcessor));


//         // extract($aMappedDataBeforeValidation, EXTR_OVERWRITE);
//         if (preg_match('/^d4/', strtolower($aRecord))) {
//             $RecordNo++;
//             $SerialNumber++;
//             $SerialNumber = str_pad($SerialNumber++, $SerialNumberOfDigits, '0', STR_PAD_LEFT);
//             $aMappedData[] = $aMappedDataBeforeValidation;
//         }
//     }
//     return $aMappedData;
